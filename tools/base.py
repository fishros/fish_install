...
class ChooseTask(Task):
    ...
    def __choose(data, tips, array):
        ...
        while True:
            if choose_item:
                ...
                print("Default option found for you from the configuration file:", choose_item)
            else:
                choose = input("Please enter the number within the brackets to choose:")
                choose_item = None
            # Input From Queue
            if choose.isdecimal():
    ...
    def run(self):
        PrintUtils.print_delay("RUN Choose Task:[Please enter the number within the brackets]")
...
class ChooseWithCategoriesTask(Task):
    ...
    def __choose(data, tips, array, categories):
        ...
        tool_ids = [0]
        # Print the classification results of different types of tools
        ...
        while True:
            if choose_item:
                ...
                choose_id = input("Please enter the number within the brackets to choose:")
                choose_item = None
            # Input From Queue
...
class FileUtils():
    ...
    def new(path,name=None,data=''):
        PrintUtils.print_info("Create File:{}".format(path+name))
...
class AptUtils():
    ...
    def checkapt():
        ...
        if result[0] != 0:
            if FileUtils.check_result(result, ['certificate', '证书']):
                PrintUtils.print_warn("Detected certificate verification error {}, automatically disable HTTPS verification, if necessary, please manually delete: rm /etc/apt/apt.conf.d/99verify-peer.conf".format(result[2]))
                ...
                result = CmdTask('sudo apt update', 100).run()
        if result[0] != 0:
            PrintUtils.print_warn("apt update failed, subsequent programs may continue to try...,{}".format(result[2]))
            return False
        return True
    ...
    def getArch():
        ...
        if result[0] == 0:
            return arc
        PrintUtils.print_error("Prompt: Automatic retrieval of system architecture failed... Please select manually")
        # @TODO Provide architecture options amd64, i386, arm
        return None
    ...
    def search_package(name, pattern, replace1="", replace2=""):
        ...
        if result[0] != 0:
            PrintUtils.print_error("Unable to find any {} related packages".format(name))
            return None
        dic = {}
    ...
    def install_pkg(name, apt_tool="apt", auto_yes=True, os_command=False):
        ...
        if not result:
            PrintUtils.print_warn("Package not found: {}".format(name))
        return result

    @staticmethod
    def install_pkg_check_dep(name):
        """
        Install and check dependency issues
        """
        result = AptUtils.install_pkg(name)
        if result:
            # Automatically agree to install once
            ...
            # Still not working, let the user install manually
            while FileUtils.check_result(result, ['unmet dependencies']):
                ...
                PrintUtils.print_delay("Please pay attention to me, detect that you have dependency problems during the installation process, please select a solution later (the first solution may not solve the problem, if encountered again, you can use the next solution), you can solve it")
                input("Confirm that you understand the above situation, please press Enter to continue the installation")
...
class BaseTool():
    ...
    def init(self):
        PrintUtils.print_delay("Welcome to use {}, this tool is provided by the author {}".format(self.name, self.author))
    
    def run(self):
        PrintUtils.print_warn("Empty tool")
